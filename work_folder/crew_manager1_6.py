...
class HealthCrewManager:
    ...
    def prepare_inputs(self, query: str) -> dict:
        """Prepare all required inputs including template variables"""
        return {
            'query': query,
            # Template variables for tasks
            'data': 'No specific data provided',
            'mealplan': 'Current meal plan to be updated',
            'exerciseplan': 'Current exercise plan to be updated', 
            'summary': 'Summary to be generated',
            'weekly_progress_summary': 'Weekly progress summary to be generated',
            'onboarding_data': 'Initial onboarding data to be collected',
            'crisis_response_log': 'Crisis response documentation',
            'quality_assurance_log': 'Quality assurance documentation',
            'weekly_progress_report_YYYY_MM_DD': 'Weekly progress report with timestamp',
            'monitoring_period': 'Current monitoring period',
            'data_quality_issue': 'Data quality issue to be addressed'
        }
    ...
    def process_query(self, query: str) -> dict:
        ...
        try:
            ...
            # Execute the crew with the user query and all required inputs
            print(f"Processing query: {query[:50]}...")
            
            # Prepare all inputs including template variables
